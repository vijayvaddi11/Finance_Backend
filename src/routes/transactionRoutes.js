...
router.get("/", verifyJWT, async (req, res) => {
  try {
    ...
    res
      ...
      .json({ msg: "Error viewing transaction" });
  }
})
...
router.post("/create", verifyJWT,  async (req, res) => {
  try {
      ...
      switch (k) {
        ...
        case "type": {
          if (!["income", "expense"].includes(v)) {
            return res
              .status(StatusCodes.BAD_REQUEST)
              .json({ msg: "type can either be income or expense only" });
          }
          break;
        }
        case "date": {
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ msg: "Error creating transaction" });
  }
});
  ...
  try {
    ...
    if (response.rowCount > 0) {
      return res
        ...
        .json({ msg: `Transaction deleted successfully for id: ${id}` });
    } else {
      return res
