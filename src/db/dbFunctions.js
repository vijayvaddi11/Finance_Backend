export const fetchUserDetailsByEmail = async (client, email) => {
	const resp = await client.query(`SELECT * FROM users WHERE email = $1`, [email]);
	return resp;
};
...
export const fetchTransactionsById = async (client, transactionId) => {
	const resp = await client.query(`SELECT * FROM transactions WHERE id = $1`, [
		transactionId,
	]);
	if (resp && resp.rows) {
		return resp.rows[0];
...
export const viewTransactions = async (client, role, userId, filters) => {
	...
	if (filters.sortBy && allowedSortFields.includes(filters.sortBy)) {
		const order =
			filters.order && filters.order.toLowerCase() === 'desc' ? 'DESC' : 'ASC';
...
export const fetchAll = async (client) => {
	return await client.query(`SELECT * FROM transactions WHERE is_active = true`);
};
